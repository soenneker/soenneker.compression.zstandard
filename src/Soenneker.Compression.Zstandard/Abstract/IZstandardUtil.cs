...
public interface IZstandardUtil
{
    ...
    /// <param name="destination">Buffer to write decompressed data into.</param>
    /// <param name="written">Number of bytes written when successful.</param>
    /// <returns><c>true</c> if decompression succeeded; <c>false</c> when the destination is too small.</returns>
    /// <remarks>Malformed or unsupported Zstandard frames still throw.</remarks>
    [Pure]
    ...
    /// <remarks>The compressed and decompressed contents are buffered in memory. The destination is replaced only after decompression succeeds.</remarks>
